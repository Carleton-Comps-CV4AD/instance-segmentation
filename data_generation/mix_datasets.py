...
'''
A file for mixing existing datasets to aid domain adaptation
'''
...
def mix_data(splits):
    '''
    the idea here is that we shall go to the paths,
...
if __name__ == '__main__':
    # weather : percentage
    {'clear_day' : 70,'foggy_day' : 30}, # done
    splits = [
              {'clear_day' : 70,'rainy_day' : 30},
            {'clear_day' : 70,'clear_night' : 30},
            {'clear_day' : 40,'clear_night' : 20, 'foggy_day': 20, 'rainy_day': 20},
            {'clear_day' : 70,'clear_night' : 10, 'foggy_day': 10, 'rainy_day': 10}]
