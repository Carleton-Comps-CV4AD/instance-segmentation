...
for video_file in "${video_files[@]}"; do
  ...
  for model_config in "${model_configs[@]}"; do
    ...
    output_file="/Data/video_data/video_results/${model_name}_on_${video_name}.mp4"

    if ! python demo/mot_demo.py "$video_file" $config_checkpoint --out "$output_file" 2> error.log; then
